...
Opts& depth::Opts::useInflater(InflaterRef inflaterRef) {
  this->inflaterRef = inflaterRef; return *this;
}
...
void depth::convert_32bit_to_8bit(size_t size, const void* data, void* target) {
  ...
  for(int i=0; i<size; i++) {
    ...
    ((unsigned char*)target)[i] = (unsigned char)((double)val / multiplier);
  }
}

void depth::convert_16bit_to_8bit(size_t size, const void* data, void* target) {
  ...
  for(int i=0; i<size; i++) {
    ...
    ((unsigned char*)data)[i] = (unsigned char)((double)val / multiplier);
  }
}

// void convert_8bit_to_16bit(size_t amount, const void* data, void* target) {
//   double multiplier = (2^16) / (2^8);
//
//   for(int i=0; i<amount; i++) {
//     // read 8-bit value
//     int byte0 = ((const unsigned char*)data)[i];
//     // convert to 32-bit int and scale
//     unsigned int val = (byte0 & 0xFF) * multiplier;
//     // write to 16-bit target buffer
//     ((unsigned char*)target)[i*2  ] = (val >> 8) & 0xFF;
//     ((unsigned char*)target)[i*2+1] = val & 0xFF;
//   }
// }


FrameRef convert_32bit_to_8bit(size_t size, const void* data) {
  void* converted = malloc(size);
  convert_32bit_to_8bit(size, data, converted);
  return Frame::refWithData(converted, size);
}

FrameRef convert_16bit_to_8bit(size_t size, const void* data) {
  void* converted = malloc(size);
  convert_16bit_to_8bit(size, data, converted);
  return Frame::refWithData(converted, size);
}
